...
VDrumMelody : Object {
  ...
  *new{ arg channel;
  ...
  playNote{ arg vel;
	...
	//oct = [0.125, 0.25, 0.5, 2, 4].choose;
	oct = 1;
	//	("chose a freq: "++freq).postln;
	//("got a vel: "++vel).postln;
	freq = freq*oct;
	Synth("simpleFormant", [\frequency, freq, \attackTime, 0.3.rand, \vel, vel, \start, 10000.0.rand, \end, 2000.0.rand]);
	//Synth("simpleNoise", [\frequency, freq*0.5, \outBus, [0,1], \attackTime, 1.0.rand]);
	//Synth("simpleNoise", [\frequency, freq, \outBus, [0,1], \attackTime, 1.0.rand]);
  ...
  sendSynthDefs{
	...
	SynthDef(
	  "simpleNoise", {arg frequency=220, outBus=0, attackTime=0.01, sustainTime=0.2, releaseTime=0.5;
		...
		chain = Resonz.ar(ClipNoise.ar(1.0), frequency, 0.0003, mul:100)*env;
		Out.ar([0,1], chain);
	  }
	).send(server);
	...
	SynthDef("simpleFormant", {arg frequency=220, outBus=1, attackTime=0.01, sustainTime=0.1, releaseTime=0.2, vel, start, end;
	  ...
	  midiTo1 = 1/127;
	  level = vel*midiTo1;
	  env = EnvGen.ar(                                                                                  		Env.new([0, 1, 0.2, 0], [attackTime, 0.1*level+0.1, 0.1*level], 'welch'), levelScale:level+0.2*0.8, doneAction:2);
	  sound = Formant.ar([frequency, frequency*0.5], XLine.kr(start, end, 0.1), XLine.kr(end, start, 0.1), 0.125);
