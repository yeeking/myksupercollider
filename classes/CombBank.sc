	...
	sendSynthDefs{
		...
		SynthDef(
			"combBank", { arg audioInBus, outBus, note1Set, note2Set, note3Set, note4Set, decaySet,lagSet;
				var combArray1, combArray2, source, delay, decay, decayScalar, lagScalar, lagCtl, midiTo1, max_dt, min_dt, lfo;
				midiTo1 = 1/127;
				source = AudioIn.ar(audioInBus)*0.5;
				//source = source + PinkNoise.ar(0.01);
				// gate it
				...
				combArray1=[
					...
					LFDNoise1.ar(0.5, mul:0.5, add:0.5)*(CombN.ar(source,0.1, Lag.kr(note2Set, lagCtl), decay)+source)].mean;
				combArray2=[
					LFDNoise1.ar(0.5, mul:0.5, add:0.5)*(CombN.ar(source,0.1, Lag.kr(note3Set, lagCtl), decay)+source),
					LFDNoise1.ar(0.5, mul:0.5, add:0.5)*(CombN.ar(source,0.1, Lag.kr(note4Set, lagCtl), decay)+source)].mean;

				combArray1 = Compander.ar(combArray1, combArray1,thresh:0.09,slopeBelow:0.1,slopeAbove:0.5,clampTime:0.01,relaxTime:0.2);
				combArray2 = Compander.ar(combArray2, combArray2,thresh:0.09,slopeBelow:0.1,slopeAbove:0.5,clampTime:0.01,relaxTime:0.2);

				lfo = SinOsc.ar(0.1).range(0.2, 1.0);
				Out.ar(outBus, [combArray1 * lfo, combArray2 * lfo])
			}
