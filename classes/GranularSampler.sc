...
GranularSampler2 : GranularSampler{
	...
	setSpeed{arg val;
		play_synth.set(\playback_position, val*2);
	}
	setPitch{arg val;
		play_synth.set(\grain_pitch, val);
	}
	setGrainCount{arg val;
		play_synth.set(\grain_count, val);
	}
	setGrainLength{arg val;
		play_synth.set(\grain_length, val)
	}
	//		  0, {play_synth.set(\grain_length, val);grain_length=val;},
	//	  1, {play_synth.set(\grain_count, val);grain_count=val;},
	//	  2, {play_synth.set(\grain_pitch, val);grain_pitch=val;},
...
GranSamplerLPD : GranularSampler2 {
	...
	*new {arg start_pad = 0, out=0;
		^super.new.prInit(start_pad);
	}

	prInit{arg start_pad;
		lpd = MPD.new(lpd_mode:1);
		samp = GranularSampler2.new(setup_midi:0);
		{samp.startRecord}.defer(0.5);
		{samp.stopRecord}.defer(1.0);
		lpd.onNoteOn(1 + start_pad, {samp.startRecord});
		lpd.onNoteOn(2 + start_pad, {samp.stopRecord});
		lpd.onNoteOn(3 + start_pad, {samp.startPlay});
		lpd.onNoteOn(4 + start_pad, {samp.stopPlay});
		lpd.onCC(1 + start_pad, {arg val; samp.setSpeed(val)});
		lpd.onCC(2 + start_pad, {arg val; samp.setPitch(val)});
		lpd.onCC(3 + start_pad, {arg val; samp.setGrainCount(val)});
		lpd.onCC(4 + start_pad, {arg val; samp.setGrainLength(val)});
	}
